...
export async function runReview(id, { resume = true } = {}) {
  const r = await fetch(`/api/review/${encodeURIComponent(id)}/run`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ resume }),
  });
  if (!r.ok) throw new Error((await r.json().catch(() => ({}))).error || 'failed to start runner');
  return r.json();
}
